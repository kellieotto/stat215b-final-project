...
# (U, V, R, Q, W1, W2, W3) are multivariate normal
a <- c1 <- d <- 1
c2 <- 2
e1 <- e2 <- e3 <- 0.5
f1 <- g2 <- 0.25
...
for(i in 1:sims){
  ...
  popsize <- 30000
  rctpopsize <- 5000
  rctsample <- sample(1:popsize, rctpopsize)
  ...
  rct <- dat[rctsample,]
  rct <- rct[rct$S == 1,]
  rct$X <- sample(c(0,1), size = nrow(rct), replace = TRUE)
  rct$Xobs <- ifelse(rct$C == 1, rct$X, 0)
  
  # Predict who is a complier using W1, W2, W3
  suppressWarnings(complier_mod <- randomForest(C~W1+W2+W3, data = rct)) # estimate propensity of compliance
  rct$C_pscore <- predict(complier_mod, rct)
  rct$Chat <- rep(1, nrow(rct))
  rct$Chat[rct$C == 0] <- as.numeric(rct$C_pscore[rct$C == 0] >= 0.5)
  dat$C_pscore <- predict(complier_mod, dat)
  compliers <- rct[rct$Chat == 1,]
  
  
  # Set up the non-randomized trial. 
  nrt <- dat[observsample,]
  nrt <- nrt[nrt$S==0,]
  nrt$Chat <- rep(1, nrow(nrt))
  nrt$Chat[nrt$C == 0] <- as.numeric(nrt$C_pscore[nrt$C == 0] >= 0.5)
  ...
  response_mod <- randomForest(Y~X + W1 + W2 + W3, data = compliers)
...
#  term1 <- mean(nrt_compliers$C_pscore[nrt_compliers$X == 1]*nrt_compliers$Yhat[nrt_compliers$X==1])
#  term2 <- mean(nrt_compliers$C_pscore[nrt_compliers$X == 0]*nrt_compliers$Yhat[nrt_compliers$X==0])
  ...
  true_patt[i] <- mean(b[X == 1])
  # SATE
  rct_sate[i] <- mean(rct$Y[rct$X == 1]) - mean(rct$Y[rct$X==0])
