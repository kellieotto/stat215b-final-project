...
sate.het <- lapply(y.col, function (i) lapply(covs, function(x) (mean(Y.ohie[[i]][which(treatment.ohie==1)]) - 
                                                                   mean(Y.ohie[[i]][which(treatment.ohie==0)])) 
                                              /mean(rct.compliers$complier[which(treatment.ohie==1)]))) # heterogenous treatment effect on sample treated compliers
