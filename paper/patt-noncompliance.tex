...
Let $Y_{ist}$ be the potential outcome for individual $i$ in group $s$, where $s=0$ for the population and $s=1$ for the randomized control trial, and $t$ be the treatment assigned.  Let $T_i$ denote the treatment assigned and $D_i$ denote treatment received. \textcolor{red}{Let $W_i$ be individual $i$'s observable covariates related to the sample selection mechanism for membership in the RCT. } Let $C_i$ be an indicator for individual $i$'s compliance to treatment.  For a generic value, we drop the subscript $i$.  \\
...
\todo{mention distinction between $S=0$ and $S=1$. In practice, data for $S=0$ comes from a non-randomized trial, in which the participants are representative of the target population}
...
\end{tikzpicture}
\caption{Causal diagram which satisfies the conditional independence assumptions needed to estimate the PATT.}\label{fig:DAG}
\end{figure}
...
Figure~\ref{fig:sim_compliance} compares the mean squared error of the two PATT estimators and the SATE (as an estimator of PATT) at varying levels of compliance in the total population.  When compliance is low, both PATT estimators have a large mean squared error and variance.  The adjusted estimator has a large variance because the subset of the RCT who are considered compliers is relatively small, whereas the unadjusted estimator has large variance because of the large amount of crossover.  \todo{For low levels of compliance, SATE actually tends to approximate PATT more closely; the variance due to small sample size is less than the noise added by non-compliance in the other two estimators.}  On the other hand, when the compliance rate is high, both PATT estimators perform about equally well with mean squared error close to $0$.  The median mean squared error for SATE stays relatively constant regardless of compliance rate
...
\subsection{Assumptions}
\todo{Discuss how the assumptions for estimating PATT are met or not met by the data}
\subsection{Results}
\todo{}
...
\item In the OHIE trial, only about $30\%$ of those selected to receive Medicaid benefits actually enrolled successfully, and successful enrollment depended in part on income levels.  The proposed estimator that adjusts for non-compliance seems to perform better than the unadjusted estimator when compliance is low and can be predicted by observed covariates (Figure~\ref{fig:sim_compliance}).  This gives evidence in favor of using the proposed estimator in this situation.
\end{itemize}
